 ...
 * @file			iodefine.h
 *
 * @brief			Registerdefinitionen und Makros für den Registerzugriff.
 *
 * @details			An dieser Datei habe ich keine Änderungen am
 * 					Quelltext vorgenommen.
 *
 ********************************************************************************
 *
 * @par         Verlauf
 ...
 * History    : V1.8C  (2014-07-18)  [Hardware Manual Revision : 1.80]
 ...
 *            : V1.0E  (2012-06-09)  [Hardware Manual Revision : 1.00]
 *            : V1.0D  (2012-06-17)  [Hardware Manual Revision : 1.00]
 * Note       : This is a typical example.
 * Copyright  : 2012 Renesas Electronics Corp. And Renesas Solutions Corp.,
 *              All Rights Reserved.
 * </pre>
 *
 ********************************************************************************
 ...
 * for example :
 *              IR(TPU0,TGI0A) = 0; expands to : ICU.IR[126].BIT.IR = 0;
 *              DTCE(ICU,IRQ0) = 1; expands to : ICU.DTCER[64].BIT.DTCE = 1;
 *              IEN(CMT0,CMI0) = 1; expands to : ICU.IER[0x03].BIT.IEN4 = 1;
 *       		IPR(SCI12,SCIX0) = 2; expands to : ICU.IPR[122].BIT.IPR = 2;
 *       		IPR(SCI0,RXI0) = 3; expands to : ICU.IPR[214].BIT.IPR = 3;
 *  Usage : @code{.c}  #pragma interrupt Function_Identifier(vect=**) @endcode
 *     The number of vector is "(interrupt source, name)".
 *     for example :
 *       @code{.c} #pragma interrupt INT_IRQ0(vect=VECT(ICU,IRQ0)) @endcode   expands to :
 *       @code{.c}   #pragma interrupt INT_IRQ0(vect=64)  @endcode
 *       @code{.c} #pragma interrupt INT_CMT0_CMI0(vect=VECT(CMT0,CMI0))  @endcode   expands to :
 *       @code{.c}   #pragma interrupt INT_CMT0_CMI0(vect=28)  @endcode
 *       @code{.c} #pragma interrupt INT_MTU0_TGIA0(vect=VECT(MTU0,TGIA0))  @endcode  expands to :
 *       @code{.c}   #pragma interrupt INT_MTU0_TGIA0(vect=142) @endcode
 *       @code{.c} #pragma interrupt INT_TPU0_TGI0A(vect=VECT(TPU0,TGI0A))   @endcode expands to :
 *       @code{.c}   #pragma interrupt INT_TPU0_TGI0A(vect=126)  @endcode
